...
class TestFiles(unittest.TestCase):
    ...
    def create_temp_file(self, file_path_suffix: str, content: bytes):
        file_path = "temp_file"
        filename = "_".join([file_path, file_path_suffix])
    ...
    def test_empty_file(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b""
        )
    ...
    def test_small_file_within_chunk_size(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b"hello"
        )
        result = self.files.get_file_sha256(file_path)
    ...
    def test_file_exactly_chunk_size(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b"A" * CHUNK_SIZE
        )
        result = self.files.get_file_sha256(file_path)
    ...
    def test_file_larger_than_two_chunks(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), content
        )
    ...
    def test_file_exactly_two_chunks(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b"A" * (CHUNK_SIZE * 2)
        )
    ...
    def test_single_byte_file(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b"A"
        )
        result = self.files.get_file_sha256(file_path)
    ...
    def test_large_file(self):
        ...
        file_path = self.create_temp_file(
            datetime.datetime.now().strftime(DATE_FORMAT), b"A" * (CHUNK_SIZE * 20)
        )
